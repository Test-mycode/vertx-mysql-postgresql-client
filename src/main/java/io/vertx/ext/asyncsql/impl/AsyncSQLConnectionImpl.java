...
public abstract class AsyncSQLConnectionImpl implements SQLConnection {
  ...
  private SQLConnection endAndStartTransaction(String command, Handler<AsyncResult<Void>> handler) {
    if (inTransaction) {
      inTransaction = false;
      ConversionUtils.completableFutureToVertx(connection.sendQuery(command), vertx).onComplete(
          ar -> {
            if (ar.failed()) {
              handler.handle(Future.failedFuture(ar.cause()));
            } else {
              ConversionUtils.completableFutureToVertx(connection.sendQuery(getStartTransactionStatement()), vertx).onComplete(
                  ar2 -> {
                    if (ar2.failed()) {
                      handler.handle(Future.failedFuture(ar.cause()));
                    } else {
